...
Gui 1: Add, Button, x12 y209 w160 h30 gDuyet, Duyệt
...
Gui 1: Add, Text, x12 y489 w170 h20 vEstimated2, Thời gian 1 dòng:
...
Gui 5: Add, Button, x610 y10 w100 h30  gSuacatalog, suacatalog
...
Loop
{
	...
	If (varID = "")
	{
		...
		sleep, 400
		;check map
		ImageSearch, Xcheckmap,,218, 750 ,1061, 977, PNG\chuamap.png
		if (Xcheckmap != "")
		{
			MsgBox, Map loi
		}
...
*/
activebienthesp:= 1870068
activebienthett:= 1071644
gosub, count
Loop
{
	starttime:=starttime()
	...
	if (fdcount>=1)
	{
		...
		Loop
		{
			PixelSearch, Px, Py, 621, 584, 1255, 606, 0x2c1beb ,3, fast
			Px := Px + 5
			Py := Py + 5
			click, %Px%, %Py%, left, 1
			If ErrorLevel
		...
		; ảnh 1 -> click picture -> đổi identifier -> Save
			...
			for index, element in XAnh_All
			{
				...
				clickretry(1106, 241)
				sleep, 200
				checkmau(1108, 231,0xF1EDEA)
				sleep,200
				click, 803, 394 left, 3
				sleep, 100
				send, %varID%_%sttanh%
				sleep, 200
				checkmau2(1059, 254,0xE3DAD2)
				clickretry(1059, 248)
				sleep, 400
					;MsgBox, check
			}
...
Loop, %varFolder%\*.png
	{
		...
		IfInString, A_Loopfilename, renamed
			continue
		IfNotExist, %varFolder%\%jpgname%.jpg
		{
				FileCopy, %A_LoopFilefullpath%, %A_temp%\%A_loopfilename%
				;MsgBox, %A_temp%\%A_loopfilename%
				RunWait, topng.exe `"%A_temp%\%A_LoopFileName%`" jpg
				namenoext := StrSplit(A_loopfilename,".")[1]
				FIleCopy, %A_temp%\%namenoext%.jpg,%A_LoopFileDir%,1
				FileMove, %A_LoopFilefullpath%, %A_Loopfiledir%\%namenoext%_renamed.png
				FileDelete,%A_temp%\%namenoext%.jpg
				FileDelete,%A_temp%\%A_loopfilename%
		}
	}
Loop, %varFolder%\*.jpeg
	{
		StringReplace, jpgname, A_Loopfilename, jpeg, jpg
		IfInString, A_Loopfilename, renamed
			continue
		IfNotExist, %varFolder%\%jpgname%.jpg
		{
				...
				namenoext := StrSplit(A_loopfilename,".")[1]
				FIleCopy, %A_temp%\%namenoext%.jpg,%A_LoopFileDir%,1
				FileMove, %A_LoopFilefullpath%, %A_Loopfiledir%\%namenoext%_renamed.jpeg
				FileDelete,%A_temp%\%namenoext%.jpg
				FileDelete,%A_temp%\%A_loopfilename%				
		}
	}	
...
GuiControl, 1:, GuiRow , Dòng số: %Row%/%Count%
...
GuiControl, 1:, Estimated,% "Còn lại: "timedisplay.hour ":" timedisplay.min ":" timedisplay.sec 
GuiControl, 1:, Estimated2,% "Last time :" timedisplay.lastrowtime " Best time: " timedisplay.besttime
}
return
...
Loop
{
	...
	else
	{
		...
		CheckMau(1900,883,0xFFFFFF,200)
		sleep, 200
		click, 1843, 800 left, 1
		sleep, 400
		click, 1446, 801 left, 1
		checkmau2(1503, 964,0xFFFFFF)
		click, 1519, 827 left ,1
		sleep, 200
		click , 1252, 953 left , 2
		sleep, 200
		checkmau(1263, 445,0xFFFFFF)
		sleep,200
		...
		sleep, 300
		click, 1241, 245 left, 1
		checkmau(1900,883,0xFFFFFF)
		sleep, 200
		click, 1904, 156 left, 1
		sleep, 200
